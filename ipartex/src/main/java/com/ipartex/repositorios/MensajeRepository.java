...
//	Iterable<MensajeDTO> listarMensajes(@Param("email") String email);
	...
	@Query("select new com.ipartex.dtos.MensajeDTO(m.id, m.usuario.id, m.usuario.nombre, m.usuario.usuarioIpartex, m.texto, :usuario member of m.lesGusta, size(m.lesGusta), size(m.respuestas), m.fecha) from Mensaje m where m.respuestaDe is null order by m.fecha desc")
