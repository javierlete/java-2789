...
public interface MensajeRepository extends CrudRepository<Mensaje, Long> {
	...
	@Query("select new com.ipartex.dtos.MensajeDTO(m.id, m.usuario.nombre, m.usuario.usuarioIpartex, m.texto, :usuario member of m.lesGusta, size(m.lesGusta), m.fecha) from Mensaje m where m.respuestaDe is null order by m.fecha desc")
